...
//  private serviceAccount = require('./my-special-way-android-firebase-adminsdk.json');
  ...
  async sendTxtMsgToAndroid(clientToken: string, messageBody: string, messageTitle: string): Promise <boolean> {
    ...
    try {
      ...
      this.logger.log('FCMSender:: success in sending text message to client with token: ' && clientToken && '. Message ID: ' && response);
      return true;
    } catch (err) {
      this.logger.log('FCMSender:: failure in sending text message to client with token: ' && clientToken && '. Error: ' && err);
      // write to log
      return false;
    }
  }
