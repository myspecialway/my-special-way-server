...
describe('student permission service', () => {
    ...
    it('should call getById function and return students on validateObjClassMatchRequester', async () => {
        ...
        await studentPermissionService.validateObjClassMatchRequester(DBOperation.READ, {class_id: '123'}, MOCK_TEACHER_CONTEXT);
    ...
    it('should call getStudentsByClassId function and return students on getAndValidateStudentsInRequesterClass', async () => {
        (usersPersistence.getById as jest.Mock).mockReturnValue(Promise.resolve({class_id: '123'}));
        (usersPersistence.getStudentsByClassId as jest.Mock).mockReturnValue(Promise.resolve([, [{ username: 'test' }]]));

        const response = await studentPermissionService.getAndValidateStudentsInRequesterClass(DBOperation.READ, null, MOCK_TEACHER_CONTEXT);
        expect(response).toEqual([Permission.OWN, [{ username: 'test' }]]);
        expect(usersPersistence.getStudentsByClassId).toHaveBeenCalled();
    });

    it('should call getStudentsByClassId function and throw exception on getAndValidateStudentsInRequesterClass with diff class_id', async () => {
        (usersPersistence.getById as jest.Mock).mockReturnValue(Promise.resolve({class_id: '234'}));
        (usersPersistence.getStudentsByClassId as jest.Mock).mockReturnValue(Promise.resolve([, [{ _id: '101', username: 'test' }]]));

        let err = false;
        try {
            const response = await studentPermissionService.getAndValidateStudentsInRequesterClass(DBOperation.READ, '123', MOCK_TEACHER_CONTEXT);
        } catch (e) {
            err = true;
        }
        expect(usersPersistence.getStudentsByClassId).toHaveBeenCalled();
    ...
    it('should call getStudentsByClassId and return students on getAndValidateStudentsInRequesterClass with requested class_id', async () => {
        (usersPersistence.getById as jest.Mock).mockReturnValue(Promise.resolve({class_id: '234'}));
        (usersPersistence.getStudentsByClassId as jest.Mock).mockReturnValue(Promise.resolve([, [{ _id: '123', username: 'test' }]]));
    ...
    it('shouldnt call getStudentsByClassId and getById, return ALLOW with no students on getAndValidateStudentsInRequesterClass', async () => {
        (usersPersistence.getById as jest.Mock).mockReturnValue(Promise.resolve({class_id: '234'}));
        (usersPersistence.getStudentsByClassId as jest.Mock).mockReturnValue(Promise.resolve([, [{ _id: '123', username: 'test' }]]));

        const response = await studentPermissionService.getAndValidateStudentsInRequesterClass(DBOperation.READ, '123', MOCK_PRINCIPLE_CONTEXT);
