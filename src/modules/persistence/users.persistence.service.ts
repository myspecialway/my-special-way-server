...
export class UsersPersistenceService extends Logger {
  ...
  async getByUsername(username: string): Promise<[Error, UserDbModel]> {
    try {
      ...
      this.error(
        `getAll:: error fetching user by username ${username}`,
        error.stack,
      );
      return [error, null];
    }
  ...
  async getClassStudents(class_id: string) {
    try {
